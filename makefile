...
test-integration:
	pytest --cov=src/vineapp --cov-report=term -m "integration"
coverage:
	pytest --cov=src/vineapp --cov-report=term --cov-report=html
build:
	python -m build
printversion:
...
docker_image:
	docker build -t ghcr.io/serraict/vine-demo-app:$(VERSION) .
docker_push: docker_image
	docker push ghcr.io/serraict/vine-demo-app:$(VERSION)
